...
'''
Kafka consumer for consuming from the topic "OSPF_NEIGHBOR_UP".
Events in that topic are generated by napalm-logs (syslog).
The events are correlated using the correlate_identical function
to check if all OSPF neighbors have come back up.
'''
