...
class Correlate:
    ...
    @classmethod
    def aggregate(cls, data, host, timestamp, severity, error, sensor_type,
        ...
        if cache is None or cache_id not in cache or error not in cache[cache_id]:
            ...
            cls.__init_cache(error, cache_id, correlate_for)
        else:
        ...
        if cache[cache_id][error]['counter'] == n_of_events:
            if use_oats_case:
                cls.__update_db_case(current_case, cache[cache_id][error]['counter'], event_name, event_name)
            ...
            if use_oats_case:
                ...
                cls.__update_db_case(current_case, cache[cache_id][error]['counter'], event_name, alternative_event_name)
    ...
    @staticmethod
    def __update_db_case(self, current_case, counter, event_name, identifier):
        oatspsql.update_case(current_case,
