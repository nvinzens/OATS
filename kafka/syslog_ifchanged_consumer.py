...
'''
Kafka consumer for consuming from the topic "INTERFACE_CHANGED".
Events in that topic are generated by napalm-logs (syslog).
The Events are correlated using the aggregate_distinct function,
to discern between simple interface_down events and
port_flapping events.
'''
