...
'''
Kafka consumer for consuming from the topic "OSPF_NEIGHBOR_DOWN".
Events in that topic are generated by napalm-logs (syslog).
The Events are correlated using the aggregate_identical function
to check if an OSPF process has died.
'''
